(function () {
  ...
  const countyModal = document.querySelector("#county-modal");
  const loadButton = document.querySelector("#load-button");
  const resetButton = document.querySelector("#reset-button");

  // Show county modal on page load
  window.onload = function () {
    countyModal.style.display = "block";
  };
  ...
  // Format strings to their proper title case
  ...
  async function fetchData() {
    ...
    return { data: csv, counties }; // Return both datasets
  }

  function populateCountyDropdown(counties) {
    counties.features.forEach((feature) => {
      const option = document.createElement("option");
      option.value = feature.properties.NAME; // Assuming 'NAME' holds county names
      option.textContent = feature.properties.NAME; // Display county name
      countySelect.appendChild(option);
    });
  }

  resetButton.addEventListener("click", async function () {
    // Clear existing crash data from the map
    if (map.getSource("counties")) {
      map.removeLayer("counties"); // Remove the crashes layer
      map.removeSource("counties"); // Remove the crashes source
    }

    // Reset the county selection
    countySelect.value = ""; // Clear the selected county

    // Reset the title in the h1
    h1.innerHTML = "Kentucky Crash Data &ndash; 2023";

    // Reset the map view to the initial position
    map.setCenter([-85.5, 38.02]); // Reset to the original center
    map.setZoom(7); // Reset zoom level

    // Show the county selection modal again
    countyModal.style.display = "block"; // Reopen the county modal
  });

  // Load data when user clicks load button
  loadButton.addEventListener("click", async function () {
    const county = countySelect.value; // Get the selected county

    const { data, counties } = await fetchData();

    // Find the selected county in the GeoJSON
    const selectedCounty = counties.features.find(
      (feature) => feature.properties.NAME === county // Direct comparison
    );

    if (selectedCounty) {
      const bounds = turf.bbox(selectedCounty);
      map.fitBounds(bounds, { padding: 50 });

      // Filter the data for the selected county
      const filteredData = data.filter(
        (row) => row.County === county // Direct comparison
      );

      createGeoJson(filteredData, selectedCounty);

      // Update the h1 with the selected county
      h1.innerHTML = `${toTitleCase(
        selectedCounty.properties.NAME
      )} County Crash Data &ndash; 2023`;

      countyModal.style.display = "none"; // Close the modal after selection
    } else {
      alert("County not found. Please select a valid county.");
    }
  });
  ...
  const map = new maplibregl.Map({
    container: "map",
    style:
      "https://api.maptiler.com/maps/0196b0e2-ea56-44ac-bf94-c9fb230df9ad/style.json?key=VR7FKTd6lXA4PKRQVzfY",
    center: [-85.5, 38.02],
    zoom: 7,
    minZoom: 6,
  });
  ...
  function createGeoJson(filteredData, selectedCounty) {
    const geojson = {
